...
function BuyPage() {
  ...
  useEffect(() => {
    // Fetch the fruit data based on the new "id" parameter
    const fetchedFruit = getFruitById(id);
    setFruit(fetchedFruit);

    // Fetch and set the suggestions based on the new "id" parameter
    const fetchedSuggestions = getOtherFruitSuggestions(fetchedFruit);
    setSuggestions(fetchedSuggestions);
  }, [id]); // Watch for changes in the "id" parameter

  // Render the component only when the fruit data is available
  if (!fruit) {
    return null;
  }
